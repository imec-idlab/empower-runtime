...
# Sandbox APPs
#./empower-runtime.py apps.sandbox.managers.txpolicymanager.txpolicymanager --tenant_id=$TENANT_ID apps.managers.slicemanager --tenant_id=$TENANT_ID apps.managers.macmanager --tenant_id=$TENANT_ID apps.sandbox.managers.flowmanager.flowmanager --tenant_id=$TENANT_ID apps.sandbox.managers.wifislicemanager.wifislicemanager --tenant_id=$TENANT_ID --minimum_quantum=$MINIMUM_QUANTUM --quantum_decrease_rate=$QUANTUM_DECREASE_RATE --quantum_increase_rate=$QUANTUM_INCREASE_RATE apps.sandbox.managers.mcdahandovermanager.mcdahandovermanager --tenant_id=$TENANT_ID --descriptor=$MCDA_DESCRIPTOR --db_monitor=$DB_MONITOR apps.handlers.lvapstatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR apps.handlers.wifistatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR apps.handlers.ncqmstatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR apps.handlers.ucqmstatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR apps.handlers.binstatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR  apps.handlers.slicestatshandler --tenant_id=$TENANT_ID --db_monitor=$DB_MONITOR
